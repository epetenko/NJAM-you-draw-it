...
c.svg.append('circle').attr('cx',c.totalWidth*.17-13).attr('cy',c.height*.48).attr('r', 5).attr('class', 'intro-dot')

c.svg.append('text').attr('x',c.totalWidth*.17-5).attr('y',c.height*.475).text('Start dragging here').attr('class','intro-text')
...
var clipRect = c.svg
  ...
  .append('rect')
  .at({width: c.x(2007)-2, height: c.height})
...
var drag = d3.drag()
  .on('drag', function(){
    ...
    var year = clamp(2008, 2016, c.x.invert(pos[0]))
