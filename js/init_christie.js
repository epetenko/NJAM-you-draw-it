...
{'year': 2010, 'rating': 45.7},
{'year': 2011, 'rating': 50.5},
{'year': 2012, 'rating': 56.7},
{'year': 2013, 'rating': 68},
{'year': 2014, 'rating': 49.1},
{'year': 2015, 'rating': 37.8},
{'year': 2016, 'rating': 26.6},
{'year': 2017, 'rating': 17.1}
...
c.svg.append('circle').attr('cx',c.totalWidth-winwidth).attr('cy',c.height*.45).attr('r', 5).attr('class', 'intro-dot')

c.svg.append('text').attr('x',c.totalWidth-winwidth+5).attr('y',c.height*.42).text('Start dragging here').attr('class','intro-text')


c.x.domain([2010, 2017])
c.y.domain([10, 75])
...
var drag = d3.drag()
  .on('drag', function(){
    ...
    var year = clamp(2011, 2017, c.x.invert(pos[0]))
    ...
    if (!completed && d3.mean(yourData, ƒ('defined')) == 1){
      console.log(yourData[yourData.length-1])
      completed = true
      clipRect.transition().duration(1000).attr('width', c.x(2017))
      d3.select('#answer-4').style('visibility', 'visible').html("<div>You guessed <p class='your-pink'>"+ d3.format(",.3r")(yourData[yourData.length-1].rating) + "</p> percent for 2017.</div><div>The real value was <p class='your-pink'>"+d3.format(",.3r")(data[7].rating)+" </p> percent.</div>")
